...
/*   Updated: 2017/10/25 11:35:18 by cuzureau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_btree				*ft_create_node(t_dir *d, char *param)
{
	...
	if (S_ISDIR(d->infos.st_mode) &&
		...
		node->isdir = '0';
	if (d->opt[0] == 1)
		node->long_list = ft_long_listing(d, param);
	if (d->opt[4] == 1)
	{
	...
	if (!d->infos.st_mode)
	{
		...
		ft_strcpy(&node->print[ft_strlen(node->print)], ": No such file or directory\n");
	}
	return (node);
}
...
int					ft_create_tree(t_dir *d, char *current)
{
	...
	while ((d->file = readdir(d->dir)) != NULL)
	{
		...
		if (ft_strequ(d->file->d_name, ".") || ft_strequ(d->file->d_name, "..")
			...
			d->opt[4] == 1 ? ft_by_date(&d->node, d, d->file->d_name) : ft_by_name(&d->node, d, d->file->d_name);
	}
