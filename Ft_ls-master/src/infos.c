...
/*   Updated: 2017/10/23 17:21:15 by cuzureau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				ft_count_len(t_dir *d, char **long_l)
{
	...
	while (i < 8)
	{
		...
		{
			printf("long_l[%d] = [%s]\n", i, long_l[i]);
			printf("	i=%d | strlen=%zu | max=%d\n", i, ft_strlen(long_l[4]), d->max_len[4]);
		}*/
	...
	//printf("strlen=%zu\n", ft_strlen(long_l[4]));
	//printf("long_l[4]= [%s] len=%d\n", long_l[4], d->max_len[4]);
}
...
char				**ft_long_listing(t_dir *d, char *param)
{
	...
	if (S_ISLNK(d->infos.st_mode))
		param ? (readlink(param, l[7], 1024)) : (readlink(d->path, l[7], 1024));
